...
def object_ee_distance(
    ...
    object_ee_distance = (
        torch.norm(asset_pos - object_pos[:, None, :], dim=-1).max(dim=-1).values
    )
...
def contacts_dexee(env: ManagerBasedRLEnv, threshold: float) -> torch.Tensor:
    """Penalize undesired contacts as the number of violations that are above a threshold."""

    f0_contact_sensor: ContactSensor = env.scene.sensors["F0_J3_jointbody_object_s"]
    f1_contact_sensor: ContactSensor = env.scene.sensors["F1_J3_jointbody_object_s"]
    f2_contact_sensor: ContactSensor = env.scene.sensors["F2_J3_jointbody_object_s"]
    # check if contact force is above threshold
    f0_contact = f0_contact_sensor.data.force_matrix_w.view(env.num_envs, 3)
    f1_contact = f1_contact_sensor.data.force_matrix_w.view(env.num_envs, 3)
    f2_contact = f2_contact_sensor.data.force_matrix_w.view(env.num_envs, 3)

    f0_contact_mag = torch.norm(f0_contact, dim=-1)
    f1_contact_mag = torch.norm(f1_contact, dim=-1)
    f2_contact_mag = torch.norm(f2_contact, dim=-1)
    good_contact_cond1 = (f0_contact_mag > threshold) & (
        (f1_contact_mag > threshold) | (f2_contact_mag > threshold)
    )
...
def position_command_error_tanh(
    ...
    return (1 - torch.tanh(distance / std)) * contacts_dexee(env, 1.0).float()
...
def orientation_command_error_tanh(
    ...
    return (1 - torch.tanh(quat_distance / std)) * contacts_dexee(env, 1.0).float()
