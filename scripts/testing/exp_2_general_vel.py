...
# wait a frame to ensure the physics simulation is initialized
    ...
    try:
        ...
        spatial_velocity_w_w0 = - mr.VecToso3(rb0_velocities_np[0,3:]) @ pos_w0 + rb0_velocities_np[0,:3]
        twist_w_w0 = np.hstack([rb0_velocities_np[0,3:] , spatial_velocity_w_w0])
        
        R_w1, pos_w1 = mr.TransToRp(H_w1)
        spatial_velocity_w_w1 = - mr.VecToso3(rb1_velocities_np[0,3:]) @ pos_w1 + rb1_velocities_np[0,:3]
        ...
        print(f"Origin Velocity from Spatial Vel ver 1: {(mr.VecTose3(twist_0_01) @ mr.TransInv(H_w0) @ np.hstack([pos_w1,[1]])).round(4)}")
        print(f"{'PhysX':=^20}")
...
def main():
    ...
    while simulation_app.is_running():
        ...
        transform_matrix = rigidBodyXform[0].ComputeLocalToWorldTransform(Usd.TimeCode.Default())
        transform = Gf.Transform(transform_matrix)
        H_w0 = np.array(transform.GetMatrix()).T
        transform_matrix = rigidBodyXform[1].ComputeLocalToWorldTransform(Usd.TimeCode.Default())
        transform = Gf.Transform(transform_matrix)
        ...
        print(f"RPY [rad/s]: {(sm_quat_rot.rpy('rad')/(time + 1e-4)).round(4)}, rate ang_vec [rad/s]: {(sm_quat_rot.angvec('rad')[0] * sm_quat_rot.angvec('rad')[1])/(time + 1e-4)}")
        print(f"{'=':=^20}")
        ...
        rigidBodyCircleVelocities = Gf.Vec3f(-1.0, 0.0, 0.0) # Gf.Vec3f(0.0, 0.0, 0.5*np.cos(0.1*time))
        rigid_api[1].GetVelocityAttr().Set(rigidBodyCircleVelocities)
        
        # rigidBodyCircleVelocities = Gf.Vec3f(0.0, 0.0, 0.5*np.sin(0.1*time))
        rigidBodyCircleVelocities = Gf.Vec3f(0.0, 0.0, 0.0) # Gf.Vec3f(0.0, 0.0, 0.5*np.cos(0.1*time))
