...
def get_dir_paths_to_obj(
    ...
    for o_dir in obj_directories:
        ...
        checking_dir = (
            lambda dir: dir.is_dir() and (path_to_obj / dir / file_obj).is_file()
        )
        if include_mdls:
            checking_dir = (
                lambda dir: dir.is_dir()
                and dir.name in include_mdls
                and (path_to_obj / dir / file_obj).is_file()
            )

        model_directories = [
            dir.name for dir in path_to_obj.iterdir() if checking_dir(dir)
        ]
...
def main():
    ...
    for o_cls in obj_classes:
        ...
        list_obj_dir_models = [
            dir.name
            for dir in path_to_obj_cls.iterdir()
            if dir.is_dir() and dir.name.split("_")[-1].isdigit()
        ]
        dict_db_structure[o_cls] = sorted(
            list_obj_dir_models, key=lambda x: int(x.split("_")[-1])
        )

    path_usd_dataset = (
        args_cli.path_usd
    )  # "/home/yefim-home/Documents/work/IsaacGraspingEnv/source/IsaacGraspEnv/IsaacGraspEnv/assets/data/HANDEL"
    ...
    for o_cls, models in dict_db_structure.items():
        ...
        if mass_prop_obj:
            for obj_model in models:
                path_to_obj = (
                    path_to_dataset / o_cls / obj_model / args_cli.obj_file_name
                )  #'object_convex_decomposition_meter_unit.obj'

                # TODO: Remove Copy Labeled PC
                path_to_pc_labeled = (
                    path_to_dataset / o_cls / obj_model / "point_cloud_labeled.ply"
                )
                ...
                sdf_collision_props = ext_schemas_cfg.SDFCollisionPropertiesCfg(
                    sdf_bits_per_subgrid_pixel="BitsPerPixel16"
                )
                collision_props = ext_schemas_cfg.ExtendedCollisionPropertiesCfg(
                    contact_offset=0.001,
                    rest_offset=0.0001,
                    sdf_collision_properties_cfg=sdf_collision_props,
                )
                ...
                mesh_converter_cfg = MeshConverterCfg(
                    ...
                    rotation=R.from_euler("xyz", [0.0, 0.0, 0.0], degrees=True)
                    .as_quat()[[3, 0, 1, 2]]
                    .tolist(),
                )

                mesh_converter = ExtentedMeshConverter(
                    mesh_converter_cfg,  # geom_name=f"{o_cls}_{obj_model}"
                )

                if args_cli.name_point_cloud:
                    path_to_pc = (
                        path_to_dataset / o_cls / obj_model / args_cli.name_point_cloud
                    )
