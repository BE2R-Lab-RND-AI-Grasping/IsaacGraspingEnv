...
from IsaacGraspEnv.dataset_managers import (
    load_object_dataset,
    preprocessing_point_cloud_loading,
)
...
def fabric_func_unpack_obs(env, name_obs_vector):
    ...
    id_obs_vector = env.observation_manager.active_terms["policy"].index(
        name_obs_vector
    )
    ...
    unpack_vectors4obs = lambda env: env.observation_manager._obs_buffer["policy"][
        :, index_obs_vector : index_obs_vector + size_obs
    ]
...
def main():
    ...
    if args_cli.task == "Isaac-Full-Obj-PC-Lift-Iiwa-IK-Rel-v0":
        env_cfg.observations.policy.point_cloud.params["path_to_point_clouds"] = (
            preprocessing_point_cloud_loading(
                args_cli.dataset_path, "point_cloud_colorless.ply", dt_models_filter
            )
        )
